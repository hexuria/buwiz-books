...
test.describe("Employees Detail View", () => {
  ...
  test("should select a card and show detail panel", async ({ page }) => {
    // The employees feature uses a list of buttons instead of table rows
  ...
  test("should display expected detail sections", async ({ page }) => {
    ...
    if (await firstCard.isVisible()) {
      ...
      // Note: Employees do NOT display Vendor Details or Payment Info sections.
    }
  });
});
