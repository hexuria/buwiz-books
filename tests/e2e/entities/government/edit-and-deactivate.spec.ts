...
test.describe("Government Edit and Deactivate", () => {
  ...
  test("should transition to edit mode from detail view", async ({ page }) => {
    ...
    if (await firstCard.isVisible()) {
      ...
      if (await editBtn.isVisible()) {
        ...
        const editHeader = page.getByText("Edit Government", { exact: true }).first();
  ...
  test("should surface deactivation modal from detail view", async ({ page }) => {
    ...
    if (await firstCard.isVisible()) {
      ...
      if (await deactivateBtn.isVisible()) {
        ...
        await expect(dialog.getByText("Deactivate Government")).toBeVisible();
