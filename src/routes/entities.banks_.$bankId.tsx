/**
 * Bank Detail Page (Transactions View)
 * Parity with `departments_.$departmentId.tsx`
 * Route: /entities/banks_/$bankId
 */
import { createFileRoute, Link } from "@tanstack/react-router";
import { useQuery } from "@tanstack/react-query";
import { useState, useMemo } from "react";
import { getFinancialAccount } from "./api/-financial-accounts";
...
export const Route = createFileRoute("/entities/banks_/$bankId")({
...
// ── Area Chart (Green Theme for Banks) ──
...
function BankAreaChart({
  ...
  // THEME: Green for Banks (#22c55e / #16a34a)
  return (
    <svg
    ...
    >
      ...
      <defs>
        <linearGradient id="bankAreaGrad" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="rgba(34, 197, 94, 0.25)" />
          <stop offset="100%" stopColor="rgba(34, 197, 94, 0.02)" />
        </linearGradient>
      </defs>
      <path d={areaPath} fill="url(#bankAreaGrad)" />
...
function BankSidebar({
  ...
  const shareUrl = `${typeof window !== "undefined" ? window.location.origin : ""}/entities/banks/${account.id}`;
  ...
  return (
    <div className="w-full bg-white dark:bg-[#1e293b] rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.08)] flex flex-col h-full overflow-hidden">
      {/* Tab bar — Green gradient for Banks */}
      <div className="flex justify-evenly bg-gradient-to-r from-[#15803d] to-[#22c55e] dark:from-[#14532d] dark:to-[#166534] shrink-0 px-4 py-3 gap-2">
        {tabs.map(({ key, icon }) => (
          <button
            key={key}
            type="button"
      ...
      <div className="flex-1 overflow-y-auto p-5">
        {activeTab === "details" && (
          <div className="flex flex-col items-center pt-4">
            <div className="w-16 h-16 rounded-full bg-green-100 text-green-600 flex items-center justify-center text-2xl font-bold mb-4">
            ...
            <div className="mt-6 w-full space-y-4 text-left">
              <div>
                <label className="text-xs text-[#64748b] font-medium block mb-1">Institution</label>
                <div className="text-sm text-[#1e293b] dark:text-slate-300">
              ...
              <div>
                <label className="text-xs text-[#64748b] font-medium block mb-1">Last Four</label>
        ...
        {activeTab === "comments" && <CommentThread entityType="bank" entityId={account.id} />}
        {activeTab === "people" && (
...
function BankDetailPage() {
  ...
  const { data: bank, isLoading: bankLoading } = useQuery({
    queryKey: ["financialAccount", bankId],
    queryFn: () => getFinancialAccount({ data: { id: bankId } }),
    enabled: !!bankId,
  });

  const summaryDateFrom = `${selectedYear}-01-01`;
  const summaryDateTo = `${selectedYear}-12-31`;

  const { data: summary } = useQuery({
    queryKey: ["accountTransactionSummary", bank?.ledgerAccountId, selectedYear],
    queryFn: () =>
      getAccountTransactionSummary({
        data: {
          id: bank?.ledgerAccountId ?? "",
  ...
  const { data: transactionsData } = useQuery({
    queryKey: ["bankTransactions", bank?.ledgerAccountId, dateFrom, dateTo],
    queryFn: () =>
      listTransactions({
        data: { accountId: bank?.ledgerAccountId ?? undefined, dateFrom, dateTo, limit: 250 },
      }),
    enabled: !!bank?.ledgerAccountId,
  });
  ...
  if (bankLoading) {
    return <EntityDetailLoading back={backNav} message="Loading bank details..." />;
  }
      ...
      return (
        <div className="text-center py-12">
          <p className="text-sm text-[#94a3b8] dark:text-slate-500">
            No transactions found for this bank during the selected period.
          </p>
        </div>
      );
    }
  ...
  return (
    <EntityDetailLayout
      ...
      side={<BankSidebar account={bank} />}
      sideTitle="Account details"
    >
      {/* Green Gradient Header for Banks */}
      <div className="bg-gradient-to-r from-[#15803d] to-[#22c55e] dark:from-[#14532d] dark:to-[#166534] px-5 pt-4 pb-4 shrink-0">
        {/* Row 1: Avatar + Name */}
        <div className="flex items-center gap-3 mb-3">
          <MultiAvatar
            size="sm"
            items={[{ initials: bank.accountName.substring(0, 2).toUpperCase() }]}
          />
          <h1 className="text-lg font-bold text-white">{bank.accountName}</h1>
        </div>
        ...
        <div className="flex items-center gap-3 h-8 relative">
          ...
          {/* CENTER: Placeholder for future filters */}
          <div className="flex-1 flex justify-center z-10"></div>
      ...
      {/* Tab Content */}
      <div className="overflow-y-auto flex-1">{renderTransactionContent()}</div>
    </EntityDetailLayout>
  );
}
