...
import {
  getDepartment,
  getDepartmentTransactionSummary,
...
export const Route = createFileRoute("/departments_/$departmentId")({
...
// ── Area Chart (Blue Theme for Departments) ──
const MONTH_SHORT = [
  "Jan",
  "Feb",
  "Mar",
...
function DepartmentAreaChart({
  ...
  // THEME: Blue for Departments (#3b82f6 / #0ea5e9)
  return (
    <svg
    ...
    >
      ...
      <defs>
        <linearGradient id="deptAreaGrad" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="rgba(59, 130, 246, 0.25)" />
          <stop offset="100%" stopColor="rgba(59, 130, 246, 0.02)" />
        </linearGradient>
      </defs>
      <path d={areaPath} fill="url(#deptAreaGrad)" />
...
// ── Category Tree (Placeholder / Future Use) ──
...
function DepartmentSidebar({
  ...
  const shareUrl = `${typeof window !== "undefined" ? window.location.origin : ""}/departments/${department.id}`;
  ...
  return (
    <div className="w-full bg-white dark:bg-[#1e293b] rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.08)] flex flex-col h-full overflow-hidden">
      ...
      <div className="flex justify-evenly bg-gradient-to-r from-[#0369a1] to-[#0ea5e9] dark:from-[#0c4a6e] dark:to-[#0284c7] shrink-0 px-4 py-3 gap-2">
        {tabs.map(({ key, icon }) => (
          <button
            key={key}
            type="button"
      ...
      <div className="flex-1 overflow-y-auto p-5">
        {activeTab === "details" && (
          <div className="flex flex-col items-center pt-4">
            <div className="w-16 h-16 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center text-2xl font-bold mb-4">
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`w-2 h-2 rounded-full ${
                      department.isActive ? "bg-emerald-500" : "bg-slate-300"
                    }`}
                  />
        ...
        {activeTab === "comments" && (
          <CommentThread entityType="department" entityId={department.id} />
        )}
        {activeTab === "people" && (
...
function DepartmentDetailPage() {
  ...
  const backNav: EntityDetailBack = {
    ...
    to: "/departments",
    hoverClassName: "hover:text-blue-700 dark:hover:text-blue-400",
  };
  ...
  const { data: department, isLoading: deptLoading } = useQuery({
    ...
    queryFn: () => getDepartment({ data: { id: departmentId } }),
    enabled: !!departmentId,
  });

  // Parties for Combobox
  /* Removed mutations and edit logic */

  const { data: summary } = useQuery({
    queryKey: ["departmentTransactionSummary", departmentId, selectedYear],
    queryFn: () =>
      getDepartmentTransactionSummary({ data: { id: departmentId, year: selectedYear } }),
    enabled: !!departmentId,
  });

  const { data: categories } = useQuery({
    queryKey: ["departmentCategories", departmentId, selectedYear],
    queryFn: () => getDepartmentCategories({ data: { id: departmentId, year: selectedYear } }),
    enabled: !!departmentId && activeTab === "categories",
  });

  const { data: transactionsData } = useQuery({
    queryKey: ["departmentTransactions", departmentId, dateFrom, dateTo],
    queryFn: () =>
      listTransactions({ data: { departmentIds: [departmentId], dateFrom, dateTo, limit: 250 } }),
    enabled: !!departmentId && activeTab === "transactions",
  });
  ...
  const monthlyData = useMemo(() => {
    ...
    // Note: summary.months is 1-indexed (1=Jan)
    // If range spans multiple years, this simple logic assumes we're viewing the target year.
    // Ideally we'd use year+month, but the current summary is just for one year.
    // For "Year to Date" (Jan-Today), this works perfectly.
  ...
  if (deptLoading) {
    return <EntityDetailLoading back={backNav} message="Loading department..." />;
  }
                ...
                return (
                  ...
                  >
                    ...
                    <span className="col-span-2 text-xs text-[#94a3b8] truncate">
                      {tx.departmentName || department.name}
                    </span>
                    {/* Amount */}
  ...
  return (
    <EntityDetailLayout
      ...
      side={<DepartmentSidebar department={department} />}
      sideTitle="Department details"
    >
      {/* Blue Gradient Header */}
      <div className="bg-gradient-to-r from-[#0369a1] to-[#0ea5e9] dark:from-[#0c4a6e] dark:to-[#0284c7] px-5 pt-4 pb-4 shrink-0">
        {/* Row 1: Avatar + Name */}
        <div className="flex items-center gap-3 mb-3">
          <MultiAvatar
            size="sm"
            items={[{ initials: department.name.substring(0, 2).toUpperCase() }]}
          />
          <h1 className="text-lg font-bold text-white">{department.name}</h1>
        </div>
      ...
      <div className="overflow-y-auto flex-1">
        ...
        {activeTab === "categories" && (
          <div className="p-0">
            <CategoryTree categories={(categories as CategoryNode[]) || []} />
          </div>
        )}
      </div>
        ...
        <div className="flex flex-1 items-center justify-evenly">
          {(["transactions", "categories"] as const).map((tab) => (
            <button
              ...
              style={activeTab === tab ? { backgroundColor: "rgba(59,130,246,0.12)" } : undefined}
