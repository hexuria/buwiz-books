...
import {
  ...
  DEFAULT_ENTITLEMENT_GRACE_DAYS,
...
function lockedTransitionAtEvent(
  ...
  return {
    ...
    endsAt,
    // Preserve an already-expired boundary, but never retain a future grace
    // date or violate the entitlement grace >= end constraint.
...
function preserveContinuousNonWritableEpisode(
  ...
  if (transition.status === "active" || !previous) {
    return transition;
  }
  ...
  if (
    ...
    // An entitlement that never became writable, or is already effectively
    // locked, cannot gain access from a provider failure signal. Normalize
    // nullable/future deadlines to no later than this event.
    return lockedTransitionAtEvent(transition, previous, eventCreatedAt);
  }
  if (effectivePrevious.status === "active") return transition;

  // A later failure signal must not buy another grace period. This also
  // retains the original deadline after request-time resolution has made an
  // expired stored grace row effectively locked. Only a verified incoming
  // active/trialing transition above starts a new episode.
  return {
    ...transition,
    startsAt: earlierDate(transition.startsAt, previous.startsAt),
...
export function resolveEnterpriseStripeTransition(
  ...
  if (snapshot.providerStatus === "incomplete") {
    return preserveContinuousNonWritableEpisode(
      {
        // `pending` can become active when startsAt passes. A provider-side
        // incomplete subscription must never auto-activate without a later paid
        // event, so persist the fail-closed state instead.
        status: "locked",
        startsAt: snapshot.currentPeriodStart,
        endsAt: eventCreatedAt,
