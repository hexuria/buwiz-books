...
`src/lib/inbox/review-rule-catalog.ts`. It is wired into `db:fresh`, `db:test:fresh`, the deploy
workflow and `make migrate`, so a rebuilt database is never left blank — but if `/review-agents`
ever renders "No review agents are set up yet", this is the first thing to check:
...
Against a non-local database, pass the URL explicitly rather than relying on the shell:

```bash
DATABASE_URL="$(grep '^DATABASE_URL:' .env.cloudrun.yaml | sed 's/^DATABASE_URL: *"*//;s/"*$//')" \
  bun db:review-rules:status
```

See [docs/DATABASE.md](./docs/DATABASE.md) for production migrations, backup/restore, and more.
...
```bash
make deploy          # Deploy via Cloud Build (no Docker needed)
make env             # Update env vars only (no rebuild)
make db-prod         # Push schema to production Neon
...
> qualifies, which is the safe default. It is deployed from Secret Manager (`deploy.yml`), so a
> value set by hand on the Cloud Run service is wiped on the next deploy.
...
`INVITE_ONLY` goes in your Cloud Run environment (`.env.cloudrun.yaml`):

```yaml
INVITE_ONLY: "true"
```

`ADMIN_EMAIL` is deployed from Secret Manager, because `deploy.yml` overwrites the service's whole
env-var and secret set on every deploy — a value set by hand on the service does not survive:

```bash
printf 'your-admin@example.com' | gcloud secrets versions add ADMIN_EMAIL --data-file=-
```
